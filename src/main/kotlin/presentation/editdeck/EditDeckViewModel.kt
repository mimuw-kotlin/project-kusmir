            ...
            is EditDeckEvent.AddCard -> {
                viewModelScope.launch {
                    ...
                    card?.let {
                        ...
                        when (target) {
                            ...
                            Deck.ListType.Sideboard ->
                                _editDeckState.value =
                                    editDeckState.value.copy(
                                        sideboard =
                                            _editDeckState.value.sideboard
                                                .toMutableDeckList()
                                                .apply { addCard(card) }
                                                .toDeckList(),
                                    )
                        ...
                        _addCardMenuState.value =
                            addCardMenuState.value.copy(
                                searchQuery = "",
                            )
                    }
                }
    ...
    fun onEvent(event: EditDeckEvent.ImportDeckEvent) {
        when (event) {
            is EditDeckEvent.ImportDeckEvent.ToggleImportPopup -> {
                _importDeckState.value =
                    importDeckState.value.copy(
    ...
    fun onEvent(event: EditDeckEvent.ChooseImageEvent) {
        when (event) {
            is EditDeckEvent.ChooseImageEvent.ImageSearch -> {
                _chooseImageState.value =
                    chooseImageState.value.copy(
                        searchQuery = event.query,
                    )
                viewModelScope.launch {
                    _chooseImageState.value =
