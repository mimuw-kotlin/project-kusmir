...
class DeckRepositoryImpl(
    ...
    override suspend fun createDeck(
        ...
        dao.insertCardsIntoMainDeck(
            mainDeck.toCardsList().map { it.id },
            deckId,
        )

        dao.insertCardsIntoSideboard(
            sideboard.toCardsList().map { it.id },
            deckId,
        )
    ...
    override suspend fun deleteDeckById(id: Long) = dao.deleteDeckById(id)

    override suspend fun deleteDeckByName(name: String) = dao.deleteDeckByName(name)
    ...
    override suspend fun saveDeck(deck: Deck) {
        ...
        dao.insertCardsIntoMainDeck(
            deck.mainDeck.toCardsList().map { it.id },
            deck.id,
        )

        dao.insertCardsIntoSideboard(
            deck.sideboard.toCardsList().map { it.id },
            deck.id,
        )
    }
